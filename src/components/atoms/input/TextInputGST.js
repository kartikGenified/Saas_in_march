...
Keyboard.addListener('keyboardDidHide',()=>{
    ...
    useEffect(()=>{
      // console.log("length of value",value, value.length)
        if(value?.length===15)
        {
          const data = {
            "gstin":value,
    ...
    return (
      <View style={{width:'100%', justifyContent:'center', alignItems:'center', marginBottom:20}}>
        ...
        {verifyGstData &&
        <View style={{marginTop:10, borderWidth:1, borderColor:'#000000', padding:20, width:'85%',}}>
        <PoppinsTextLeftMedium style={{color:'#808080'}} content = {`GSTIN ${verifyGstData?.body.GSTIN}`}></PoppinsTextLeftMedium>
        <PoppinsTextLeftMedium style={{color:'#808080'}} content = {`Business Name ${verifyGstData?.body.legal_name_of_business}`}></PoppinsTextLeftMedium>
